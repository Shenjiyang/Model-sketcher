...
then require per-view `complete_delivery.py` to return `DELIVERABLE` and
`complete_project.py` to return `PROJECT_DELIVERABLE` for all selected files. A direct XML writer,
